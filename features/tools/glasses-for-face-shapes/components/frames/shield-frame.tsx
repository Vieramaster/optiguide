import type { SVGProps } from "react"

export const ShieldFrame = (props: SVGProps<SVGSVGElement>) => (
    <svg
        xmlns="http://www.w3.org/2000/svg"
        width="62"
        height="26"
        fill="none"
        viewBox="0 0 62 26"
        {...props}
    >
     
        <g id="frame_shape-Sports-Shield-Wraparound">
            <g id="_x3C_Mirror_Repeat_x3E_">
                <g id="Clip path group">
                    <mask
                        id="mask0_1670_57776"
                        width="29"
                        height="26"
                        x="2"
                        y="0"
                        maskUnits="userSpaceOnUse"
                        style={{ maskType: "luminance" }}
                    >
                        <g id="clippath">
                            <path id="Vector" fill="#fff" d="M30.694.025H2v25h28.694z"></path>
                        </g>
                    </mask>
                    <g mask="url(#mask0_1670_57776)">
                        <g id="Group">
                            <path
                                id="Vector_2"
                                stroke="#currentColor"
                                strokeMiterlimit="10"
                                strokeWidth="0.75"
                                d="M30.693 1.2c-.935-.022-2.648 0-5.612.03-5.436.084-13.404.36-16.76 1.266-1.142.302-2.145.902-3.19 1.538-.692.429-1.51 1.042-1.863 1.62-.925 1.551-.653 3.093-.77 5.206-.045 2.531-.039 6.079-.026 8.409.03.419-.048.844.065.993.091.169.205.296.364.432 1.285.957 3.103 1.405 4.742 1.775 2.265.477 4.511.766 6.786 1.178 3.252.585 8.18 1.652 9.555-.11 1.315-1.428 2.263-3.703 3.353-5.53.47-.844.906-1.604 1.276-2.188.584-1.039 1.19-1.37 2.255-1.45"
                            ></path>
                        </g>
                    </g>
                </g>
            </g>
            <g id="_x3C_Mirror_Repeat_x3E__2">
                <g id="Clip path group_2">
                    <mask
                        id="mask1_1670_57776"
                        width="31"
                        height="26"
                        x="30"
                        y="0"
                        maskUnits="userSpaceOnUse"
                        style={{ maskType: "luminance" }}
                    >
                        <g id="clippath-1">
                            <path id="Vector_3" fill="#fff" d="M61 .025H30.48v25H61z"></path>
                        </g>
                    </mask>
                    <g mask="url(#mask1_1670_57776)">
                        <g id="Group_2">
                            <path
                                id="Vector_4"
                                stroke="#currentColor"
                                strokeMiterlimit="10"
                                strokeWidth="0.75"
                                d="M30.695 1.2c.934-.022 2.648 0 5.611.03 5.437.084 13.405.36 16.76 1.266 1.143.302 2.146.902 3.191 1.538.692.429 1.51 1.042 1.863 1.62.925 1.551.653 3.093.77 5.206.045 2.531.038 6.079.026 8.409-.03.419.048.844-.065.993-.091.169-.205.296-.364.432-1.285.957-3.103 1.405-4.742 1.775-2.265.477-4.511.766-6.786 1.178-3.253.585-8.18 1.652-9.556-.11-1.314-1.428-2.262-3.703-3.352-5.53a48 48 0 0 0-1.276-2.188c-.584-1.039-1.19-1.37-2.255-1.45"
                            ></path>
                        </g>
                    </g>
                </g>
            </g>
            <path
                id="Vector_5"
                stroke="#currentColor"
                strokeLinecap="square"
                strokeLinejoin="round"
                strokeWidth="1.623"
                d="M1.81 8.691v3.006"
            ></path>
            <path
                id="Vector_6"
                stroke="#currentColor"
                strokeLinecap="square"
                strokeLinejoin="round"
                strokeWidth="1.623"
                d="M59.576 8.691v3.006"
            ></path>
            <g id="Group 633156">
                <g id="_x3C_Mirror_Repeat_x3E__3">
                    <g id="Clip path group_3">
                        <mask
                            id="mask2_1670_57776"
                            width="7"
                            height="9"
                            x="24"
                            y="12"
                            maskUnits="userSpaceOnUse"
                            style={{ maskType: "luminance" }}
                        >
                            <g id="clippath-2">
                                <path
                                    id="Vector_7"
                                    fill="#fff"
                                    d="M30.5 12.025h-5.982v8.07H30.5z"
                                ></path>
                            </g>
                        </mask>
                        <g mask="url(#mask2_1670_57776)">
                            <g id="Group_3">
                                <path
                                    id="Vector_8"
                                    stroke="#currentColor"
                                    strokeLinecap="round"
                                    strokeLinejoin="round"
                                    strokeWidth="2"
                                    d="M30.821 13.838c-1.49.821-2.938 2.733-3.96 4.258"
                                ></path>
                            </g>
                        </g>
                    </g>
                </g>
                <g id="_x3C_Mirror_Repeat_x3E__4">
                    <g id="Clip path group_4">
                        <mask
                            id="mask3_1670_57776"
                            width="7"
                            height="9"
                            x="30"
                            y="12"
                            maskUnits="userSpaceOnUse"
                            style={{ maskType: "luminance" }}
                        >
                            <g id="clippath-3">
                                <path
                                    id="Vector_9"
                                    fill="#fff"
                                    d="M36.483 12.025H30.5v8.07h5.983z"
                                ></path>
                            </g>
                        </mask>
                        <g mask="url(#mask3_1670_57776)">
                            <g id="Group_4">
                                <path
                                    id="Vector_10"
                                    stroke="#currentColor"
                                    strokeLinecap="round"
                                    strokeLinejoin="round"
                                    strokeWidth="2"
                                    d="M30.18 13.838c1.49.821 2.938 2.733 3.96 4.258"
                                ></path>
                            </g>
                        </g>
                    </g>
                </g>
            </g>
        </g>
    </svg>
);
